...
export class AjoutComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.typematerielService
      .getAllTypemateriel()
      .subscribe((data) => (this.typemateriel = data));
    ...
    // this.materielsService.getMatrielByTypeAndMarqueAndModel(
    //   this.idType,
    //   this.idMarque,
    //   this.idModel
    // );
  ...
  private getinterventionsFromService() {
    this.interventionService.getAllIntervention().subscribe((data) => {
      ...
      for (let i = 0; i < this.interventions.length; i++) {
        this.interventions[i].dateajout = new Date(
          this.interventions[i].dateajout
        ).toLocaleString();
      }
    });
  }
  ...
  public isElementNotValid(field: string): boolean {
    return (
      this.addInterventionsForm.get(field) &&
      !this.addInterventionsForm.get(field).hasError("required") &&
      this.addInterventionsForm.get(field).invalid
    );
  }
  public onSelecteType(event) {
    ...
    this.model = [];
    if (this.addInterventionsForm.get("typemateriel").value) {
      this.materielsService
        .getMatrielByType(
          this.addInterventionsForm.get("typemateriel").value.id
        )
        .subscribe((item: any) => {
          console.log(item);
          this.marque = item;
          this.addInterventionsForm.get("marque").setValue(item.model);
        });
    }
  }
  ...
  public onSelecteMarque(event) {
    if (
      this.addInterventionsForm.get("typemateriel").value &&
      this.addInterventionsForm.get("marque").value
    ) {
      this.model = [];
      this.materielsService
        .getMatrielByMarque(this.addInterventionsForm.get("marque").value.id)
        .subscribe((item: any) => {
          console.log(item);
          this.addInterventionsForm.get("modele").setValue(item);
          this.model = item;
          console.log(this.addInterventionsForm.get("modele").value);
        });
    }
  }
